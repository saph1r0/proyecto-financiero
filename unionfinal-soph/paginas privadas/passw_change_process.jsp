        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (dbPassword.equals(currentPassword)) {
                    ...
                    if (rowsAffected > 0) {
                        out.println("<script>alert('Contraseña actualizada con éxito'); window.location='menupriv.jsp';</script>");
                    } else {
                        out.println("<script>alert('No se pudo actualizar la contraseña'); window.location='menupriv.jsp';</script>");
                    }
                } else {
                    out.println("<script>alert('La contraseña actual es incorrecta'); window.location='menupriv.jsp';</script>");
                }
            } else {
                out.println("<script>alert('Usuario no encontrado'); window.location='menupriv.jsp';</script>");
            }
        } catch (Exception e) {
            e.printStackTrace();
            out.println("<script>alert('Error al actualizar la contraseña: " + e.getMessage() + "'); window.location='menupriv.jsp';</script>");
        } finally {
